...
Object.assign(HK.CONST, { FACTOR_COST: 1500, FACTOR_WAGE: 6, COUNTERFEIT_MIN: 1000, COUNTERFEIT_MAX: 5000, PARDON_BASE: 8000, BLACKMAIL_COOLDOWN: 180 });
...
HK.blackmail = function (st, rivalId) {
  ...
  const gain = Math.round(HK.clamp(r.wealth * 0.04, 1000, 8000));
  r.wealth -= gain; r.blackmailed = st.day; r.attitude = HK.clamp(r.attitude - 40, -100, 100);
  HK.book(st, 'blackmail', gain); st.suspicion = HK.clamp(st.suspicion + 12, 0, 100); st.stats.smuggled += gain * 0.5;
...
HK.tickHooks.push(st => {
  ...
  if (st.counterfeit && st.day >= st.counterfeit.due) {
    const c = st.counterfeit; st.counterfeit = null;
    ...
    else { HK.book(st, 'counterfeit', c.amount); st.stats.smuggled += c.amount * 0.2; st.suspicion = HK.clamp(st.suspicion + 4, 0, 100); HK.log(st, 'counterfeitDone', { amount: HK.fmt(c.amount) }, 'info'); }
  }
