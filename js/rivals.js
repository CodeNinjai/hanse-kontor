...
HK.rivalHostile = function (st, r) {
  ...
  else if (a === 'poach') { const mine = Object.keys(st.ventures).filter(id => !st.ventures[id].owner && ((st.ventures[id].master && st.ventures[id].masterTrait !== 'faithful') || st.ventures[id].level > 1)); if (mine.length) { const id = HK.pick(mine); if (st.ventures[id].master) st.ventures[id].master = false; else st.ventures[id].level--; HK.log(st, 'rivalPoach', { rival: name, venture: HK.name(HK.VENTURE[id]) }, 'bad'); } else { st.suspicion = HK.clamp(st.suspicion + 4, 0, 100); HK.log(st, 'rivalDenounce', { rival: name }, 'bad'); } }
