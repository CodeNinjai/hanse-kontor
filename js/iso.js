...
'use strict';
HK.Iso = {
  TW: 26, TH: 13, OX: 450, OY: 90, ZS: 28,
  ...
  LIGHT: { top: 1.06, left: 1.0, right: 0.86, roofL: 1.0, roofR: 0.89, back: 0.76 },
  ...
  box(ctx, x, y, z0, w, d, h, col, o) {
    ...
    if (!o.noTop) this.poly(ctx, [[x, y, z0 + h], [x + w, y, z0 + h], [x + w, y + d, z0 + h], [x, y + d, z0 + h]], this.shade(col.top || col.wall, L.top), o.stroke || 'rgba(52,38,22,0.26)');
    this.poly(ctx, [[x, y + d, z0], [x + w, y + d, z0], [x + w, y + d, z0 + h], [x, y + d, z0 + h]], this.shade(col.wall, L.left), o.stroke || 'rgba(52,38,22,0.26)');
    this.poly(ctx, [[x + w, y, z0], [x + w, y + d, z0], [x + w, y + d, z0 + h], [x + w, y, z0 + h]], this.shade(col.wall, L.right), o.stroke || 'rgba(52,38,22,0.26)');
  },
  ...
  gableRoof(ctx, x, y, z, w, d, rh, col, ridge, o) {
    ...
    if (ridge === 'x') {
      ...
      if (o.gableFill) { this.poly(ctx, [[x + w, y, z], [x + w, y + d, z], [x + w, ym, z + rh]], this.shade(o.gableFill, L.right), 'rgba(52,38,22,0.28)'); this.poly(ctx, [[x, y, z], [x, y + d, z], [x, ym, z + rh]], this.shade(o.gableFill, L.left)); }
      const front = [[x - ov, ym, z + rh], [x + w + ov, ym, z + rh], [x + w + ov, y + d + ov, z - ov * 0.5], [x - ov, y + d + ov, z - ov * 0.5]];
      this.poly(ctx, front, this.shade(col, L.roofL), 'rgba(52,38,22,0.28)');
      ...
      if (o.gableFill) { this.poly(ctx, [[x, y + d, z], [x + w, y + d, z], [xm, y + d, z + rh]], this.shade(o.gableFill, L.left), 'rgba(52,38,22,0.28)'); }
      const right = [[xm, y - ov, z + rh], [x + w + ov, y - ov, z - ov * 0.5], [x + w + ov, y + d + ov, z - ov * 0.5], [xm, y + d + ov, z + rh]];
      this.poly(ctx, right, this.shade(col, L.roofR), 'rgba(52,38,22,0.28)');
  ...
  pyramid(ctx, x, y, z, w, d, rh, col) {
    ...
    this.poly(ctx, [[x + w, y, z], [x + w, y + d, z], [cx, cy, z + rh]], this.shade(col, L.roofR), 'rgba(52,38,22,0.28)');
    this.poly(ctx, [[x, y + d, z], [x + w, y + d, z], [cx, cy, z + rh]], this.shade(col, L.roofL), 'rgba(52,38,22,0.28)');
  },
  ...
  cylinder(ctx, cx, cy, z0, r, h, col, o) {
    ...
    ctx.strokeStyle = 'rgba(52,38,22,0.26)'; ctx.lineWidth = 0.8; ctx.stroke();
  ...
  cone(ctx, cx, cy, z0, r, h, col) {
    ...
    ctx.strokeStyle = 'rgba(52,38,22,0.28)'; ctx.lineWidth = 0.8; ctx.stroke();
  },
  ...
  window3(ctx, a, b, c, d, o) {
    ...
    this.poly(ctx, pts.map(q => ex(q, 0.16)), o.frame || '#775f47');
    const S = HK.Scene, lit = S && S.nightK && !S.picking && !S.emitOff && (((a[0] * 7.3 + a[1] * 5.1 + a[2] * 3.7) * 10) | 0) % 5 !== 0;
    const pane = lit ? '#d2a563' : '#3d3730', shape = o.arch ? [a, b, c, [(c[0] + d[0]) / 2, (c[1] + d[1]) / 2, c[2] + (c[2] - b[2]) * 0.35], d] : pts;
    this.poly(ctx, shape, pane);
    if (lit) S.emit(shape, `rgba(255,${160 + ((a[0] * 13) | 0) % 30},${60 + ((a[1] * 17) | 0) % 30},${0.85 * S.nightK})`);
    // Glasreflex und Kreuz
    this.poly(ctx, [a, b, [b[0], b[1], b[2] + (c[2] - b[2]) * 0.42], [a[0], a[1], a[2] + (d[2] - a[2]) * 0.42]], 'rgba(150,170,180,0.13)');
    const mid = [(a[0] + b[0]) / 2, (a[1] + b[1]) / 2, a[2]], midT = [(c[0] + d[0]) / 2, (c[1] + d[1]) / 2, c[2]];
    this.line(ctx, mid, midT, 'rgba(198,186,164,0.38)', 0.5);
    this.line(ctx, [a[0], a[1], a[2] + (d[2] - a[2]) * 0.5], [b[0], b[1], b[2] + (c[2] - b[2]) * 0.5], 'rgba(198,186,164,0.38)', 0.5);
    ...
    if (o.flowers) { this.poly(ctx, [[a[0], a[1], a[2] - 0.02], [b[0], b[1], b[2] - 0.02], [b[0], b[1], b[2] - 0.1], [a[0], a[1], a[2] - 0.1]], '#5d713e'); const P = this.p(a[0], a[1], a[2]), Q = this.p(b[0], b[1], b[2]); for (let t = 0.1; t < 1; t += 0.22) { ctx.fillStyle = ['#9f4b41', '#b6914d', '#b2705f'][Math.round(t * 10) % 3]; ctx.fillRect(P[0] + (Q[0] - P[0]) * t, P[1] + (Q[1] - P[1]) * t - 1.5, 1.6, 1.6); } }
  },
  doorL(ctx, x, y, d, z0, u, wz, hz, arch) { if (HK.Scene.picking) return; const yy = y + d; const a = [x + u - wz / 2, yy, z0], b = [x + u + wz / 2, yy, z0], c = [x + u + wz / 2, yy, z0 + hz], dd = [x + u - wz / 2, yy, z0 + hz]; this.poly(ctx, [a, b, c, dd].map(q => [q[0], q[1], q[2]]), '#745334'); const m = [(c[0] + dd[0]) / 2, yy, z0 + hz + (arch ? wz * 0.5 : 0)]; this.poly(ctx, arch ? [a, b, c, m, dd] : [a, b, c, dd], '#5b4125'); this.line(ctx, [x + u, yy, z0], [x + u, yy, z0 + hz], 'rgba(0,0,0,0.5)', 0.6); const P = this.p(x + u + wz * 0.3, yy, z0 + hz * 0.5); ctx.fillStyle = '#bb9d58'; ctx.fillRect(P[0], P[1], 1.5, 1.5); },
  doorR(ctx, x, w, y, z0, v, wz, hz, arch) { if (HK.Scene.picking) return; const xx = x + w; const a = [xx, y + v - wz / 2, z0], b = [xx, y + v + wz / 2, z0], c = [xx, y + v + wz / 2, z0 + hz], dd = [xx, y + v - wz / 2, z0 + hz]; this.poly(ctx, [a, b, c, dd], '#6c4a2f'); const m = [xx, y + v, z0 + hz + (arch ? wz * 0.5 : 0)]; this.poly(ctx, arch ? [a, b, c, m, dd] : [a, b, c, dd], '#563b23'); },
  /* Fachwerk auf einer Wand: vertikale Pfosten, Riegel, Streben */
  timberL(ctx, x, y, d, z0, w, h, n, col) { const yy = y + d; col = col || 'rgba(78,54,30,0.5)'; for (let i = 0; i <= n; i++) this.line(ctx, [x + w * i / n, yy, z0], [x + w * i / n, yy, z0 + h], col, 1); this.line(ctx, [x, yy, z0 + h * 0.5], [x + w, yy, z0 + h * 0.5], col, 0.9); this.line(ctx, [x, yy, z0 + h], [x + w, yy, z0 + h], col, 1.1); this.line(ctx, [x, yy, z0], [x + w, yy, z0], col, 1.1); this.line(ctx, [x, yy, z0 + h * 0.5], [x + w / n, yy, z0 + h], col, 0.8); this.line(ctx, [x + w, yy, z0 + h * 0.5], [x + w - w / n, yy, z0 + h], col, 0.8); },
  timberR(ctx, x, w, y, z0, d, h, n, col) { const xx = x + w; col = col || 'rgba(68,46,24,0.5)'; for (let i = 0; i <= n; i++) this.line(ctx, [xx, y + d * i / n, z0], [xx, y + d * i / n, z0 + h], col, 1); this.line(ctx, [xx, y, z0 + h * 0.5], [xx, y + d, z0 + h * 0.5], col, 0.9); this.line(ctx, [xx, y, z0 + h], [xx, y + d, z0 + h], col, 1.1); this.line(ctx, [xx, y, z0], [xx, y + d, z0], col, 1.1); },
  ...
  stepGableL(ctx, x, y, d, z0, w, rh, col, steps) { const yy = y + d, pts = [[x, yy, z0]]; steps = steps || 5; for (let i = 0; i <= steps; i++) { pts.push([x + (w / 2) * i / steps, yy, z0 + rh * i / steps]); pts.push([x + (w / 2) * (i + 1) / steps, yy, z0 + rh * i / steps]); } pts.length = pts.length - 1; for (let i = steps; i >= 0; i--) { pts.push([x + w / 2 + (w / 2) * (steps - i) / steps, yy, z0 + rh * i / steps]); pts.push([x + w / 2 + (w / 2) * (steps - i + 1) / steps, yy, z0 + rh * i / steps]); } pts.length = pts.length - 1; pts.push([x + w, yy, z0]); this.poly(ctx, pts, this.shade(col, this.LIGHT.left), 'rgba(52,38,22,0.3)'); return pts; },
  stepGableR(ctx, x, w, y, z0, d, rh, col, steps) { const xx = x + w, pts = [[xx, y, z0]]; steps = steps || 5; for (let i = 0; i <= steps; i++) { pts.push([xx, y + (d / 2) * i / steps, z0 + rh * i / steps]); pts.push([xx, y + (d / 2) * (i + 1) / steps, z0 + rh * i / steps]); } pts.length = pts.length - 1; for (let i = steps; i >= 0; i--) { pts.push([xx, y + d / 2 + (d / 2) * (steps - i) / steps, z0 + rh * i / steps]); pts.push([xx, y + d / 2 + (d / 2) * (steps - i + 1) / steps, z0 + rh * i / steps]); } pts.length = pts.length - 1; pts.push([xx, y + d, z0]); this.poly(ctx, pts, this.shade(col, this.LIGHT.right), 'rgba(52,38,22,0.3)'); return pts; },
};
