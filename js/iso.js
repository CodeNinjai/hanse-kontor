...
HK.Iso = {
  ...
  window3(ctx, a, b, c, d, o) {
    ...
    this.poly(ctx, pts.map(q => ex(q, 0.16)), o.frame || '#6b5540');
    const S = HK.Scene, lit = S && S.nightK && !S.picking && !S.emitOff && (((a[0] * 7.3 + a[1] * 5.1 + a[2] * 3.7) * 10) | 0) % 5 !== 0;
    const pane = lit ? '#e0a24e' : '#262528', shape = o.arch ? [a, b, c, [(c[0] + d[0]) / 2, (c[1] + d[1]) / 2, c[2] + (c[2] - b[2]) * 0.35], d] : pts;
    ...
    if (o.flowers) { this.poly(ctx, [[a[0], a[1], a[2] - 0.02], [b[0], b[1], b[2] - 0.02], [b[0], b[1], b[2] - 0.1], [a[0], a[1], a[2] - 0.1]], '#4c5c33'); const P = this.p(a[0], a[1], a[2]), Q = this.p(b[0], b[1], b[2]); for (let t = 0.1; t < 1; t += 0.22) { ctx.fillStyle = ['#a84438', '#b9924a', '#b2705f'][Math.round(t * 10) % 3]; ctx.fillRect(P[0] + (Q[0] - P[0]) * t, P[1] + (Q[1] - P[1]) * t - 1.5, 1.6, 1.6); } }
  },
  ...
  doorR(ctx, x, w, y, z0, v, wz, hz, arch) { if (HK.Scene.picking) return; const xx = x + w; const a = [xx, y + v - wz / 2, z0], b = [xx, y + v + wz / 2, z0], c = [xx, y + v + wz / 2, z0 + hz], dd = [xx, y + v - wz / 2, z0 + hz]; this.poly(ctx, [a, b, c, dd], '#4a3320'); const m = [xx, y + v, z0 + hz + (arch ? wz * 0.5 : 0)]; this.poly(ctx, arch ? [a, b, c, m, dd] : [a, b, c, dd], '#22160b'); },
