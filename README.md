...
Eine Wirtschaftssimulation für einen Spieler in **Sundhaven**, einer Hansestadt auf einer Halbinsel, anno 1372: Altstadt mit Markt und Kirche, Neustadt hinter der alten Nordmauer, Hafenviertel am Südkai, Handwerkerviertel mit Kloster und Spital, dahinter das Ostviertel mit Neuem Markt und Schifferhaus, Bollwerk im Südosten, Fischerdorf am Strand, ein Dorf im Umland und eine unregelmäßige Küste mit Buchten und Landzungen. Die Stadtmauer ist Backstein auf Feldsteinsockel mit Wehrgang, Schießscharten und Wassergraben; das Nordtor und das Landtor im Osten sind Torhäuser mit Zugbrücke und Vorwerk, auf dem Wehrgang gehen Wachen Streife. Das Bild füllt immer den ganzen verfügbaren Platz: Der Canvas hat kein festes Seitenverhältnis mehr, sondern wird auf die Fenstergröße eingepasst, der Anfangsblick sitzt auf der bebauten Fläche statt auf der ganzen Seekarte, und was darüber hinaus sichtbar wird, ist offene See. Zoomen und Verschieben bleiben frei. Die Stadt ist eine isometrisch gezeichnete, belebte Szene: Koggen laufen an den Stegen ein, Karawanen lagern vor dem Tor, Bürger gehen ihren Wegen nach, die Windmühle dreht sich, nachts leuchten Fenster und Laternen, im Winter liegt Schnee auf den Dächern. Jedes Gebäude und jede Person lässt sich anklicken und öffnet ein eigenes Spielsystem: Handel, Betriebe, Besitz, Geldverleih, Politik, Kirche, Korruption.

HTML, CSS und JavaScript ohne Build-Schritt: `index.html` öffnen und spielen. Gezeichnet wird über die Grafikkarte (PixiJS, als fertige Datei in `lib/` beigelegt). Oberfläche auf Deutsch und Englisch umschaltbar.

*A single-player economic simulation set in a small Hanseatic coastal town. Click buildings and people to trade, build, lend, bribe, preach and scheme your way to becoming Lord of Sundhaven. No build step; open `index.html`. Rendered on the GPU via PixiJS (vendored in `lib/`). UI in German and English.*
...
- **Gezeichnet wird auf der Grafikkarte:** Die Szene lief als Canvas-2D-Bild auf einem einzigen Prozessorkern und kostete bei 3.286×1.464 Bildpunkten 43 Millisekunden je Bild — rund fünf Bilder in der Sekunde. Der Grund war nicht die Bildgröße: Die Auflösung um ein Fünftel zu senken änderte gar nichts. Es waren die 86.000 Zeichenbefehle, die in jedem Bild neu abgesetzt wurden, für eine Stadt, die stillsteht. Jetzt liegt die Geometrie: Ein Haus wird einmal beschrieben und danach nur noch gezeigt, Schwenken und Zoomen sind bloße Verschiebungen. Der gesamte Zeichencode in `buildings.js` und `iso.js` ist dafür unverändert geblieben — er zeichnet gegen einen Adapter, der aussieht wie ein Canvas-Kontext, seine Befehle aber als PixiJS-Geometrie ablegt. Was sich bewegt, bekommt einen Aussehens-Schlüssel statt neuer Geometrie: Gleich aussehende Passanten teilen sich eine Form und werden nur verschoben. Was sich doch regt — schwankende Bäume, wehende Wimpel —, stellt der Renderer selbst fest, indem er beobachtet, wer beim Zeichnen die Uhr liest; nur diese wenigen werden aufgefrischt, und zwar über mehrere Bilder verteilt, damit kein Ruckler entsteht. Der Schlagschatten wandert mit der Sonne und wird darum aufgefangen und in jedem Bild neu gezogen. Nachts läuft dieselbe Szene ein zweites Mal als schwarze Silhouette mit leuchtenden Fenstern in eine kleine Textur, die die Grafikkarte weichzeichnet — auf der Leinwand war allein das der teuerste Posten des ganzen Bildes. Ergebnis: 11,7 Millisekunden je Bild bei nun 12 Megapixeln statt 43 bei 3,4. Die Zeichenauflösung war vorher gedeckelt und lag sogar unter der des Bildschirms; dieser Deckel ist weg.
...
js/scene.js     Szene: Kamera, Tiefensortierung, Passanten, Boden, Wetter, Trefferprüfung
lib/pixi.min.js PixiJS als Browser-Datei, beigelegt — kein Build-Schritt, kein Netz nötig
js/gfx-pixi.js  Zeichenkontext in Gestalt eines Canvas-Kontexts, der PixiJS-Geometrie ablegt
js/scene-pixi.js  Der Renderer: Ebenen, liegende Geometrie, Nachtlicht, Dunst, Wetter, Korn
...
tools/formen-test.html  Prüft alle Zeichen-Grundformen: Canvas gegen PixiJS (einfach öffnen)
